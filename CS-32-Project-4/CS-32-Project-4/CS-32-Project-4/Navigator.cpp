...
NavResult NavigatorImpl::navigate(string start, string end, vector<NavSegment> &directions) const
{
    ...
    list<NaviNode*> openList; //Should be sorted by f value (low 2 high), acts like a priority queue
    MyMap<BetterGeoCoord, double> exploredNodes;
    list<NaviNode*> toBeDeleted; //Store pointers so that they can be deleted at end of search
    ...
    while(openList.size() > 0){
        ...
        for(NaviNode neighbor : neighbors){
            ...
            double* get = exploredNodes.find(neighbor.loc);
            if(get != nullptr && *get < neighbor.f)
                continue;
            
            //Otherwise, insert into correct pos in openList s.t. openList is sorted by f value
            exploredNodes.associate(neighbor.loc, neighbor.f);
...
std::vector<NaviNode> getNeighborNodes(NaviNode* n, const SegmentMapper& sm, const GeoCoord& endCoord){
    ...
    for(StreetSegment connectingStreet : connectingStreets){
        ...
        for(Attraction a : connectingStreet.attractions){
            ...
            //a is the destination, push it as a node into neighbors
            if(isSameGeoCoord(a.geocoordinates, endCoord))
...
vector<NavSegment> getReturnRoute(NaviNode endNode){
    ...
    while (currNode->parent != nullptr) {
        ...
        //Adding proceed statement
        GeoSegment seg(currNode->parent->loc, currNode->loc);
        std::string direction = segmentDirection(seg);
        double distance = distanceEarthMiles(currNode->loc, currNode->parent->loc);
        std::string streetName = currNode->streetSeg.streetName;
        
        route.push_back(NavSegment(direction, streetName, distance, seg));
        
        //Turning onto new street, adding turn statement
        if(currNode->streetSeg.streetName != currNode->parent->streetSeg.streetName){
            ...
            route.push_back(NavSegment(turnDir, currNode->streetSeg.streetName));
        }
