...
public:
    Actor(int imageID, int startX, int startY, StudentWorld *sw, Direction dir = right, int depth = 0) : GraphObject(imageID, startX, startY, dir, depth), m_sw(sw){
        m_isAlive = true;
...
public:
    Insect(int imageID, int startX, int startY, int initialHP, StudentWorld *sw, Direction dir = right, int depth = 0) : EnergyHolder(imageID, startX, startY, initialHP, sw, dir, depth), m_stunnedTurns(0){
...
public:
    ...
    virtual void get(int damageType, int damage){};
