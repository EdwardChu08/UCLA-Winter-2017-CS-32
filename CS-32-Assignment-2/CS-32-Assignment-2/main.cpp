...
int main(){
    ...
    assert(end.get(0, test) && test == "30");
    assert(end.get(1, test) && test == "17");
    assert(end.get(9, test) && test == "17");
    assert(end.get(12, test) && test == "32");
    interleave(end, ify, end);//aliasing
    end.dump();//30 17 17 63 21 29 63 63 29 42 17 63 17 29 8 32
    assert(end.get(end.size()-1, test) && test == "32");
    assert(end.get(end.size()-4, test) && test == "17");
    interleave(end, end, end);//aliasing and same length
    end.dump();//doubles
    assert(end.get(end.size()-1, test) && test == "32");
    assert(end.get(end.size()-8, test) && test == "17");
    Sequence empty;
